...
- 📅 **日历视图** - 直观查看任务完成情况
- 🎁 **奖励商店** - 使用积分购买自定义奖励，支持单件金额与消费额度，可关闭「计入消费统计」
- 🛒 **消费统计** - 购买即消费：按商品与周期（月/年/自定义）统计消费积分与折合金额，含商品占比与可回滚的消费明细
- 📈 **分析报告** - 导出周/月/年任务执行报告（Markdown / JSON），附统计口径说明，便于结合 LLM 复盘
- 🏆 **成就系统** - AI 依据你的任务生成成就提案，接取后本地判定进度，解锁发放奖励积分（未配置 AI 时可用内置推荐成就）
- 📊 **进度统计** - 累计积分、完成任务数、购买次数、连续天数与积分明细
- 🤖 **AI 能力** - 在设置中填入 Base URL / 模型 / API Key，驱动成就生成
- 🔔 **通知提醒** - 番茄钟计时结束时发送系统通知
...
- **release 包同样可以在 `chrome://inspect` 里调试**：`src-tauri/Cargo.toml` 给 tauri 开了 `devtools` feature。wry 里唯一打开调试 socket 的 `WebView.setWebContentsDebuggingEnabled` 调用被 `#[cfg(any(debug_assertions, feature = "devtools"))]` 包着，所以只有 debug 构建自带该能力；不开这个 feature 时 release 包（以及 bow 的 device-inspect）在 `chrome://inspect` 里根本不出现——注意这不是 adb 的问题，adb 只负责传输，socket 是 app 进程自己开的。判据：`grep -c -a setWebContentsDebuggingEnabled src-tauri/target/aarch64-linux-android/release/libexp1_lib.so` 应输出 `1`（改前为 `0`）；真机上看 `adb shell cat /proc/net/unix | grep devtools` 是否有 `@webview_devtools_remote_<pid>`。删掉该 feature 即回到「只有 debug 包可调」。
