  ...
  const ModeButton = ({ targetMode }: { targetMode: PomoMode }) => {
    ...
    return (
      <button
        ...
        disabled={isRunning}
        className={`flex-1 py-3 px-4 rounded-xl font-medium text-sm transition-all ${isActive
      ...
      >
        <div className="space-y-2 max-h-80 overflow-y-auto">
          <button
            ...
            className={`w-full p-3 rounded-xl flex items-center gap-3 transition-colors ${selectedTaskId === null ? 'bg-primary/10' : 'hover:bg-surface-light'
              }`}
          >
          ...
          {tasks.filter(t => t.instance.status === 'pending').map(({ instance, template }) => (
            <button
              ...
              className={`w-full p-3 rounded-xl flex items-center gap-3 transition-colors ${selectedTaskId === instance.id ? 'bg-primary/10' : 'hover:bg-surface-light'
                }`}
            >
        ...
        <div className="space-y-6 w-80">
          ...
          <div className="space-y-2">
            ...
            <Slider
              ...
              min={2}
              max={8}
              onChange={(val) => updateSettings({ longBreakInterval: val })}
            />
          </div>
          ...
          <div className="space-y-3 pt-2 border-t border-border">
            <label className="flex items-center justify-between">
              ...
              <button
                ...
                className={`w-12 h-6 rounded-full transition-colors relative ${settings.autoStartBreaks ? 'bg-primary' : 'bg-surface-light'
                  }`}
              >
                <div className={`w-4 h-4 rounded-full bg-white absolute top-1 transition-all ${settings.autoStartBreaks ? 'left-7' : 'left-1'
                  }`} />
              </button>
            </label>
            ...
            <label className="flex items-center justify-between">
              ...
              <button
                ...
                className={`w-12 h-6 rounded-full transition-colors relative ${settings.autoStartPomos ? 'bg-primary' : 'bg-surface-light'
                  }`}
              >
                <div className={`w-4 h-4 rounded-full bg-white absolute top-1 transition-all ${settings.autoStartPomos ? 'left-7' : 'left-1'
                  }`} />
              </button>
            </label>
          </div>
